...
const CareerForm = ({ role }: { role: string }) => {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value: string) => (value.trim().length !== 0 ? null : 'Invalid Name'),
      email: (value: string) => (/^\S+@\S+\.\S+$/.test(value) ? null : 'Invalid email address'),
      phone: (value: string) =>
        /^[0-9]{10}$/.test(value) ? null : 'Invalid phone number (10 digits required)',
    },
  });
  ...
  const submitHandler = async () => {
    ...
    try {
      setIsLoading(true);
      const fileUpload = await client.assets.upload('file', userCV);
  ...
  return (
    <Box>
      <form onSubmit={form.onSubmit(submitHandler)}>
        <TextInput size="md" c="#212529" my={20} label="Name" {...form.getInputProps('name')} />
        <TextInput
          size="md"
          c="#212529"
          my={20}
